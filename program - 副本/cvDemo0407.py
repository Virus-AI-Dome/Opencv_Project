...
if __name__ == '__main__':
    ...
    string = "PIL 添加中文文字。\nby youcans from XUPT"
    pos = (50, 20)  # 字符串左上角坐标 (x,y)
    color = (0, 0, 255)  # 字体颜色 B=255，注意 PIL 使用 RGB 格式
    textSize = 50  # 字符串高度
    drawPIL = ImageDraw.Draw(imgPIL)  # 创建绘图对象
    fontText = ImageFont.truetype("font/simsun.ttc", textSize, encoding="utf-8")
    drawPIL.text(pos, string, color, font=fontText)  # 绘制字符串
    imgText = cv.cvtColor(np.asarray(imgPIL), cv.COLOR_RGB2BGR)

    plt.figure(figsize=(9, 3.5))
    plt.subplot(121), plt.title("(1) cv.putText"), plt.axis('off')
    plt.imshow(cv.cvtColor(img1, cv.COLOR_BGR2RGB))
    plt.subplot(122), plt.title("(2) PIL.Image"), plt.axis('off')
