...
if __name__ == '__main__':
    ...
    compose3 = np.zeros((h, w, 3), np.uint8)  # 创建黑色图像 BGR=0
    compose3[:, :, 0] = grayInfr  # Xray -> B
    compose3[:, :, 1] = grayOpti  # Optical -> G
    compose3[:, :, 2] = grayXray  # Infrared -> R
    ...
    plt.imshow(cv.cvtColor(compose3, cv.COLOR_BGR2RGB))
    plt.tight_layout()
    plt.show()
